...
export const signup = async(req, res, next)=>{
    try{
        ...
        if(!email || !password) return res.status(400).send('Email and Password requied');
...
export const login = async(req, res, next)=>{
    try{
        ...
        console.log(user);
        if(!user) return res.status(404).send("Can't find user with provided Email");
...
export const addProfileImage = async(req, res, next)=>{
    try{
        if(!req.file) return  response.status(400).send("File is required");
        const date = Date.now();
